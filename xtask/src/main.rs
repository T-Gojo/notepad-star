...
/// Retained Notepad++ checkout, relative to the repository root. The trees were
/// moved here wholesale when the repository was restructured around this
/// application; their contents are still byte-for-byte the baseline commit.
const UPSTREAM: &str = "reference/notepad-plus-plus";
...
/// Path of a retained upstream file, which no longer sits at the repository root.
fn upstream(relative: &str) -> PathBuf {
    workspace().join(UPSTREAM).join(relative)
}
...
fn inventory() -> Result<Inventory> {
    ...
    // The retained checkout was relocated under reference/notepad-plus-plus, so a
    // path-based diff against the baseline would report every file as added.
    // Compare the tracked blob identity instead, which ignores the move but not
    // an edit, then require the working tree to still match what is tracked.
    for relative in pinned {
        let expected = command_output(
            Command::new("git")
                .current_dir(&root)
                .args(["rev-parse", &format!("{BASELINE}:{relative}")]),
        )?;
        let tracked = command_output(
            Command::new("git")
                .current_dir(&root)
                .args(["rev-parse", &format!(":{UPSTREAM}/{relative}")]),
        )?;
        if expected != tracked {
            return Err(format!(
                "The pinned reference file {UPSTREAM}/{relative} changed; review the baseline before regenerating the inventory."
            )
            .into());
        }
    }
    ...
    worktree
        .current_dir(&root)
        .args(["--no-pager", "diff", "--quiet", "--"]);
    for relative in pinned {
        worktree.arg(format!("{UPSTREAM}/{relative}"));
    }
    checked(&mut worktree).map_err(|error| {
        format!("A pinned reference file has uncommitted edits; restore it before regenerating the inventory: {error}")
    })?;
    let mut entries: BTreeMap<String, Feature> = BTreeMap::new();
    let header = "PowerEditor/src/menuCmdID.h";
    let command_source = fs::read_to_string(upstream(header))?;
    ...
    for source in [
        ...
        let contents = fs::read_to_string(upstream(source))?;
...
fn package(
    ...
    for (source, name) in [
        ("LICENSE", "LICENSE.txt"),
        (
            "reference/notepad-plus-plus/scintilla/License.txt",
            "Scintilla-LICENSE.txt",
        ),
        (
            "reference/notepad-plus-plus/lexilla/License.txt",
            "Lexilla-LICENSE.txt",
        ),
        ("NOTICE.txt", "NOTICE.txt"),
        ("native/languages/NOTICE.txt", "LanguageAssets-NOTICE.txt"),
        ("native/function_list/NOTICE.txt", "FunctionList-NOTICE.txt"),
    ] {
        fs::copy(root.join(source), output.join(name))?;
    }
    licenses(environment, &output)?;
    fs::copy(
        root.join("reference/notepad-plus-plus/scintilla/test/unit/LICENSE_1_0.txt"),
