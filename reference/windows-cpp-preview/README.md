...
```powershell
.\reference\windows-cpp-preview\build.ps1 -Run
```
...
```powershell
.\reference\windows-cpp-preview\build.ps1 -Configuration Release -Test
```
...
```powershell
.\reference\windows-cpp-preview\build\Release\notepad-star.exe
.\reference\windows-cpp-preview\build\Release\notepad-star.exe "C:\Work\example.cpp" "C:\Work\notes.txt"
```

Open `reference\windows-cpp-preview\NotepadStar.vcxproj` in Visual Studio to develop and debug the new app.
...
| `..\notepad-plus-plus\{scintilla,lexilla,boostregex}` | Reused editor libraries, unchanged |
